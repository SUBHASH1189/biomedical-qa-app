...
def main():
    ...
    # CORRECTED LINE: We load the dataset and shuffle it.
    # We remove the .select(range(5000)) because the dataset only has 1000 entries.
    # This will now use all 1000 available examples for training and validation.
    raw_dataset = load_dataset(DATASET_NAME, DATASET_CONFIG, split='train').shuffle(seed=42)
    
    # The rest of the script works perfectly with this change.
